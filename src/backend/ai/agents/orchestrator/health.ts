...
export async function checkHealth(agent: OrchestratorAgent, env: Env) {
  ...
  try {
    const db = getDb(env);
    await db.select({ count: sql`count(*)` }).from(roles).limit(1);
...
export async function checkOrchestratorAgentRPC(env: Env) {
  const start = Date.now();
  try {
    const stub = await getAgentByName<Env, OrchestratorAgent>(env.ORCHESTRATOR_AGENT as any, "global");
    const result = await stub.healthProbe();
    if (!result || typeof result !== "object" || !("status" in result)) {
      return { status: "fail", latencyMs: Date.now() - start, error: `Invalid response from OrchestratorAgent: ${String(result)}` };
    }
    return result;
  } catch (e) {
    return { status: "fail", latencyMs: Date.now() - start, error: `OrchestratorAgent RPC failed: ${e instanceof Error ? e.message : String(e)}` };
  }
}
